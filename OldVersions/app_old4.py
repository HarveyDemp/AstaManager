...
# Funzione helper per convertire AttrDict/st.secrets in dict nativo senza RecursionError
def secrets_to_dict(secrets_obj):
    if hasattr(secrets_obj, "items"):
        ...
        return [secrets_to_dict(v) for v in secrets_obj]
    else:
        return secrets_obj


# 2. Converti le credenziali in modo sicuro
        ...
        with st.container(border=True):
            ...
            with tab_login:
                ...
                if st.session_state.get("authentication_status") is False:
                    ...
                    st.info("ℹ️ Inserisci le tue credenziali per accedere.")
            ...
            with tab_register:
                try:
                    ...
                    if username:
                        st.success(
                            f"✅ Account per **{username}** creato con successo! "
...
user_name = st.session_state.get("name", user_id)
...
if st.session_state.app_mode == "menu":
    ...
    with col_menu_logout:
        st.write("")
        authenticator.logout("🚪 Logout", "main", key="logout_menu")
...
# ==========================================
    ...
    # SIDEBAR: UTENTE + LOGOUT + PREFERITI
    with st.sidebar:
