...
# ==========================================
    ...
    # SIDEBAR PREFERITI
    with st.sidebar:
        st.title("⭐ Preferiti Asta")
        st.caption("I tuoi calciatori sotto osservazione.")
        ...
        if st.session_state.preferiti:
            ...
            for idx, p_row in pref_df.iterrows():
                ...
                status_str = f"❌ (Speso: {st.session_state.asta_state['giocatori_acquistati'][p_name]['squadra_asta']})" if is_sold else f"Qt: {p_row['Qt.A']} | FVM: {p_row['FVM']}"
                
                c_pref1, c_pref2 = st.columns([4, 1])
                c_pref1.markdown(f"**{p_name}** ({p_row['R']}) - *{p_row['Squadra']}*<br><small>{status_str}</small>", unsafe_allow_html=True)
            ...
            st.info("Nessun giocatore nei preferiti.")
    ...
    # ==========================================
        ...
        # ---- CSS: pulsante download compatto, stesso stile dei pulsanti standard ----
        st.markdown(
            """
            <style>
            ...
            [data-testid="stDownloadButton"] > button:hover {
                ...
                color: #2ecc71 !important;
                border-color: #2ecc71 !important;
            }/* X giocatore: stessa icona della squadra, ma più piccola e senza sfondo */


            [class*="st-key-iconbtn__del__player__"] .stButton > button {
                background: transparent !important;
                background-color: transparent !important;
                border: none !important;
                box-shadow: none !important;
            ...
            [class*="st-key-iconbtn__del__player__"] .stButton > button:hover,
            [class*="st-key-iconbtn__del__player__"] .stButton > button:focus,
            [class*="st-key-iconbtn__del__player__"] .stButton > button:active {
                background: transparent !important;
                background-color: transparent !important;
                border: none !important;
                box-shadow: none !important;
        ...
        # POP-UP CONFERMA ELIMINAZIONE SQUADRA
        if st.session_state.confirm_delete_team:
            target_t = st.session_state.confirm_delete_team
            st.warning(f"⚠️ Sei sicuro di voler eliminare la squadra **'{target_t}'**? Tutti i suoi calciatori torneranno nel listone liberi!")
            col_c1, col_c2 = st.columns([1, 1])
            if col_c1.button("✅ Sì, Elimina Squadra"):
                for p_name, p_info in list(st.session_state.asta_state["giocatori_acquistati"].items()):
                    if p_info["squadra_asta"] == target_t:
                        del st.session_state.asta_state["giocatori_acquistati"][p_name]
                del st.session_state.asta_state["squadre"][target_t]
                st.session_state.confirm_delete_team = None
                save_asta_to_file()
                st.rerun()
            if col_c2.button("❌ Annulla"):
                st.session_state.confirm_delete_team = None
                st.rerun()
            ...
            for col_idx, (team, data) in enumerate(teams_items[i:i+4]):
                with cols[col_idx]:
                    ...
                    with st.container(key=f"teamcard__c{color_idx}__{team}", border=True):
                        ...
                        c_title.markdown(
                            f"<div style='color:{b_color}; font-size: 1.1rem; font-weight: bold; padding-top: 4px;'>🛡️ {team}</div>",
                            unsafe_allow_html=True,
                        )
                        ...
                        # Form In-Line di Rinomina
                        if st.session_state.editing_team == team:
                            renamed_val = st.text_input("Nuovo nome:", value=team, key=f"inp_rename_{team}")
                            if st.button("Salva Nome", key=f"save_rename_{team}"):
                                if renamed_val.strip() and renamed_val.strip() not in st.session_state.asta_state["squadre"]:
                        ...
                        if total_p_count == 0:
                            ...
                            for r_code in ["P", "D", "C", "A"]:
                                ...
                                st.markdown(
                                    f"<div class='role-header'>{role_names[r_code]} ({len(r_list)}/{max_s})</div>",
                                    unsafe_allow_html=True,
                                )
                                if r_list:
                                    for p_item in r_list:
                                        ...
                                        with c_p1:
                                            st.markdown(
                                                f"<div class='player-row'>• <b>{p_item['Nome']}</b> "
                                                f"<span style='float:right; color:#2ecc71;'>{p_item['Prezzo']} cr</span></div>",
                                                unsafe_allow_html=True,
                                            )
                                            with c_p2:
                                                if st.button(
                                                    "×",    
                                                    key=f"rm_player_{team}_{p_item['Nome']}",
                                                    help="Rimuovi dalla rosa e riporta disponibile",
                                                    type="tertiary",
                                                    width="content"
                                                ):
                                                    # Rimborsa i crediti e libera il giocatore
                                                    data["crediti_residui"] += p_item["Prezzo"]
                                                    data["rosa"] = [
                                                        p for p in data["rosa"] if p["Nome"] != p_item["Nome"]
                                                    ]
                                                    if p_item["Nome"] in st.session_state.asta_state["giocatori_acquistati"]:
                                                        del st.session_state.asta_state["giocatori_acquistati"][p_item["Nome"]]
                                                    save_asta_to_file()
                                                    st.rerun()
                                else:
                                    st.markdown(
                                        "<div class='player-row' style='color:#94a3b8; font-style:italic;'>Nessuno</div>",
                                        unsafe_allow_html=True,
                                    )
